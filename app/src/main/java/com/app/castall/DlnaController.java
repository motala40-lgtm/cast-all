...
final class DlnaController {
    ...
    void setAvTransportUriAndPlay(DlnaDevice device, String mediaUrl, String title, String mimeType,
                                   String upnpClass, long durationMs, ResultCallback callback) {
        EXECUTOR.execute(() -> {
            boolean uriSet = setAvTransportUri(device, mediaUrl, title, mimeType, upnpClass, durationMs);
            if (uriSet) {
    ...
    private final AtomicLong seekGeneration = new AtomicLong(0);

    /**
     * If the user drags/adjusts the seek position more than once before an
     * earlier request has finished (including its own retries against a
     * TV that's still "Transition not available"), each new call here
     * immediately invalidates any older in-flight one — only the latest
     * position the user actually wants is worth still fighting for. Without
     * this, a burst of quick corrections all queue up and compete for the
     * TV's attention one after another, which just keeps it perpetually
     * busy and makes every one of them fail.
     */
    void seek(DlnaDevice device, long positionMs, ResultCallback callback) {
        long myGeneration = seekGeneration.incrementAndGet();
        EXECUTOR.execute(() -> {
            if (myGeneration != seekGeneration.get()) {
                return; // a newer seek request replaced this one before it even started
            }
            String target = formatTime(positionMs);
            String seekBody = "<InstanceID>0</InstanceID><Unit>REL_TIME</Unit><Target>" + target + "</Target>";
            boolean ok = sendAction(device.avTransportControlUrl, AV_TRANSPORT_SERVICE, "Seek", seekBody);
            if (myGeneration != seekGeneration.get()) {
                return; // superseded while we were retrying; don't report a stale result
            }
            postResult(callback, ok);
        });
    }
    ...
    /**
     * Diagnostic-only: reports the renderer's own CurrentTransportState
     * (e.g. PLAYING, TRANSITIONING, PAUSED_PLAYBACK). Used after a Seek
     * failure to determine whether the renderer had actually finished
     * transitioning into PLAYING yet, since UPnP's "Transition not
     * available" fault is specified to mean the requested action isn't
     * valid from whatever state the renderer currently claims to be in.
     */
    void getTransportState(DlnaDevice device, TransportStateCallback callback) {
        EXECUTOR.execute(() -> {
            ...
            String state = responseBody != null ? extractTag(responseBody, "CurrentTransportState") : null;
            String finalState = state != null ? state : "unknown (" + lastError + ")";
            mainHandler.post(() -> callback.onState(finalState));
        });
    }
    ...
    private boolean setAvTransportUri(DlnaDevice device, String mediaUrl, String title, String mimeType,
                                       String upnpClass, long durationMs) {
        String escapedUrl = escapeXml(mediaUrl);
        String metadata = buildDidlLiteMetadata(mediaUrl, title, mimeType, upnpClass, durationMs);
    ...
    private String buildDidlLiteMetadata(String mediaUrl, String title, String mimeType, String upnpClass, long durationMs) {
        ...
        // A declared duration helps some renderers treat the track as
        // properly seekable from the start; omit it entirely when we don't
        // actually know it (e.g. an arbitrary URL) rather than guessing.
        String durationAttr = durationMs > 0 ? " duration=\"" + formatTime(durationMs) + "\"" : "";
        return "<DIDL-Lite xmlns=\"urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/\" "
                ...
                + "<res protocolInfo=\"http-get:*:" + safeMimeType + ":" + additionalInfo + "\"" + durationAttr + ">" + escapeXml(mediaUrl) + "</res>"
                + "</item></DIDL-Lite>";
    }
    ...
    private String dlnaAdditionalInfoFor(String mimeType) {
        switch (mimeType) {
            ...
            case "image/png":
                return "DLNA.ORG_PN=PNG_LRG;DLNA.ORG_CI=0;DLNA.ORG_FLAGS=00900000000000000000000000000000";
            default:
